...
// class magnifier{
// public:
//     int verticle;//安装在哪个节点
//     int tranx_id;//为哪一个交易安装
// };
...
// void Graph_init(){ //对图进行初始化（如果一对顶点之间有重复的两条边，进行排序）
...
// int find_dist(int u, int v){ //u是源点，i是目标点
//     //源点本身，距离为0
//     if(u==v){
//         return 0;
//     }
//     for(int j =0 ; j< Graph[u].size();j++){ 
//         if(Graph[u][j].dst==v){ //如果源点和目标点之间存在边，返回距离
//             return Graph[u][j].weight;
//         }
//     }
//     //找不到符合条件的边
//     return INF;
...
void cost(vector<vector<int>> all_path){
    ...
    //     vector<int> path = all_path[p];
        ...
        // 加边
            ...
            for(int i=0; i<path.size(); i++){
                //需要加边
                if(edge_channel[path[i]].stat[channel_idx]==1){
                    add_edge_idx.push_back(path[i]);
                    //边数+1
                    add_edge_num++;
...
void dijkstra(transaction &tran,int begin,int end){//u是源点，求的是源点到其他顶点最短路径
    ...
    while (!q.empty()) {
        ...
        for (auto &e : Graph[v]) {
            ...
            if (dist[v] + e.weight > dist[e.dst]) continue;
...
{   
    ...
    // for(auto iter = tranx.begin();iter != tranx.end();iter++){
    //     int begin = iter->begin; //起始节点
    //     int end = iter->end; //终点
